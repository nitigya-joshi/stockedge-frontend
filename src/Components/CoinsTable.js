...
export default function CoinsTable(props) {
   ...
   let indicesText;
   const makeUrl=(coin)=>{
        imageUrl=idToUrl[coin.id]
        indicesText=(coin.id=="nifty50" || coin.id=="sensex")?"/indices":""
   }
                    ...
                    .map(coin=>{
                        return <TableRow key={coin.id} className="coinsTableRow" onClick={(e)=>navigate(`/coins/${coin.id}`)}>
                                    <TableCell component="th" scope='row' sx={{display:"flex",alignItems:'center',gap:"1.5vmax"}}>
                                        {makeUrl(coin)}
                                        <img src={`https://s3-symbol-logo.tradingview.com${indicesText}/${imageUrl}.svg`} crossOrigin="anonymous" alt={coin?.name} className="coinsTableImg" />
